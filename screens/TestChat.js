...
export default function Chat({ route, navigation }) {
  ...
  const [limit, setLimit] = useState(10); // Number of messages to load initially
  ...
  useEffect(() => {
    ...
    const onDataChange = (snapshot) => {
      ...
      setMessages(newMessages.reverse()); // Reverse the array to maintain the correct order
  ...
  const addMessage = () => {
    // Your existing addMessage logic here

    // After adding a message, scroll to the end
    if (flatListRef.current) {
      flatListRef.current.scrollToEnd({ animated: true });
    }
  };
